app.factory('JsPlumbService', function() {
	...
	JsPlumbDiagram.prototype.connect = function(connection) {
		try {
			var fromUUID = this.getOutputStreamId(connection.from.id, connection.from.stream);
			var toUUID = this.getInputStreamId(connection.to.id, connection.to.stream);
