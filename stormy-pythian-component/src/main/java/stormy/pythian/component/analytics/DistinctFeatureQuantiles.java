...
@Documentation(name = "Distinct feature quantiles", description = "Computes quantiles of each feature occurence")
public class DistinctFeatureQuantiles implements Component {

	public static final String GROUP_BY_FEATURE = "Group by";
	public static final String QUANTILES_ON_FEATURE = "Quantiles on";
	...
	@InputStream(name = "in", type = FIXED_FEATURES, expectedFeatures = { @ExpectedFeature(name = GROUP_BY_FEATURE), @ExpectedFeature(name = QUANTILES_ON_FEATURE, type = Number.class) })
	private Stream in;
	...
	private static class QuantilesAggregator implements CombinerAggregator<TDigest> {
		...
		public TDigest init(TridentTuple tuple) {
			Instance instance = Instance.from(tuple);
			Number feature = instance.getFeature(inputMapper, QUANTILES_ON_FEATURE);
